...
// import { getAllResults, getAllDistrics, getAllPrgrams } from '@/lib/appwrite';
...
//   console.log(latestAllResults,"ls");
  ...
  const calculateDistrictData = (data) => {
    ...
    data.forEach((item) => {
      ...
      districtIds.forEach(({ id, mark, category }) => {
        ...
        // Determine the category type (general or twalaba)
        const isGeneral = category?.startsWith('G') && generalSubcategories.includes(category);
        const isTwalaba = category?.startsWith('T') && twalabaSubcategories.includes(category);
  
        let gradesOnlyScore = 0;
       
        
          const gradesOnlyData = JSON.parse(item.gradesonly);

          getcat(item.itemcode, (pointCategory) => {
            gradesOnlyScore = parseInt(gradesOnlyData?.[pointCategory.point_category] || 0);
      
            if (isGeneral) {
              districtCategoryMarks[id].general[category] =
                (districtCategoryMarks[id].general[category] || 0) + mark + gradesOnlyScore;
            } else if (isTwalaba) {
              districtCategoryMarks[id].twalaba[category] =
                (districtCategoryMarks[id].twalaba[category] || 0) + mark + gradesOnlyScore;
            }
